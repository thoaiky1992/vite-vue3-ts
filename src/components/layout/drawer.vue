<template>
  <div class="h-screen w-full px-6 pt-4">
    <ul class="drawer__menu flex flex-col">
      <li
        v-for="(drawerMenuItem, index) in getDrawerMuneItems"
      ...
      >
        <router-link
          ...
          :class="{
            'flex w-full items-center justify-between rounded-xl p-2 font-bold transition duration-100 ease-in-out hover:bg-gray-200 hover:text-gray-800 hover:shadow-lg': true,
        ...
        <ul class="drawer__submenu my-1 ml-4 border-l pl-2">
          <li v-for="drawerSubmenuItem in drawerMenuItem.children" :key="drawerSubmenuItem.key">
            <router-link
              :class="{
                'mt-0 block w-full rounded-xl py-0 px-0 text-0 transition-drawer-item delay-75 duration-200 hover:bg-gray-200 hover:text-gray-800 hover:shadow-lg': true,
...
const getDrawerMuneItems = computed(() => {
...
<style>
.drawer__menu__item.active .drawer__submenu > li > a {
  font-size: 14px;
  padding-top: 0.5rem;
